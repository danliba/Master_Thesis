...
for iy=yr(1):1:yr(end)
    ...
    for imo=mo(1):1:mo(end)
        ...
        Taux = interp2(Xu, Yu, uwind', LON', LAT', 'linear')'; %N/m2
        %regrid vwind
        Tauy = interp2(Xv, Yv, vwind', LON', LAT', 'linear')'; %N/m2
...
[c,h]=contourf(timeclim,latSSH2,climCUTI,[-8:0.5:8]);shading flat; colorbar; 
cmocean('delta'); clabel(c,h);set(h,'LineColor','white'); 
title('CUTI Index Climatology 150km offshore 1 degree bin','fontsize',16); 
ax = gca;
ax.FontSize = 16;
